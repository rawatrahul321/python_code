...
class WorkedDayOvertime(models.Model):
    ...
    def get_worked_day_lines(self, contract_ids, date_from, date_to):
        ...
        for attn_lines in self.env['hr.attendance'].search([('check_in', '>=', date_from),
            ...
            print 'CHECK IN DATE TIME +++++++',check_in.date(),   check_in.time()
            time_check = datetime.strptime('04:30:00','%H:%M:%S')
            print 'TIME CHECK TIME++++++',time_check.time()
            if check_in.time() < time_check.time():
                print 'HELLO U are Early'
                check_in = check_in.replace(hour=04, minute=30)
                print 'New Check In Time FOr Early Comers',check_in
            else:
                check_in = datetime.strptime(attn_lines.check_in, '%Y-%m-%d %H:%M:%S')
                
            check_out = datetime.strptime(attn_lines.check_out, '%Y-%m-%d %H:%M:%S')
            time_diff = check_out - check_in 
            total_hours += ((time_diff.seconds / 60) / 60) - 1
            print 'Total Hours++++++++',total_hours,    normal_hours
