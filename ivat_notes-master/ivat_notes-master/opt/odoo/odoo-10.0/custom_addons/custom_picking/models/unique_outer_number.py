...
class UniqueOuterPackingNumber(models.Model):
    _name = 'unique.outer.packing.number'
    _inherit = ['mail.thread']
    _description = 'Unique Outer Packing Number'
    ...
    line_ids = fields.One2many('outer.packing.unique.traceability', 'traceability_id',string= 'Traceability')
...
class OuterPackingTraceability(models.Model):
    _name = "outer.packing.unique.traceability"
    _description = "Outer Packing unique Traceability"
    
    name = fields.Char('Description')
    traceability_id = fields.Many2one('unique.outer.packing.number', 'Unique Serial Number')
