...
class AutoGenerateOuterPackingNumber(models.Model):
    _name = 'auto.generate.outer.packing.number'
    ...
    def _default_outerCartonItems(self):
        workorder_id = self.env['mrp.workorder'].browse(self._context.get('active_id'))
        product_id = self.env['product.product'].search([('id','=',workorder_id.product_id.id)])
        return product_id.outer_carton
    ...
    outer_carton = fields.Float("Outer Carton", default=lambda self: self._default_outerCartonItems())
    ...
    def auto_generate_no(self):
        ...
        prefix = ""+ 'O~' +"" "" + ""+ prd_temp_id.custom_product_code +""+ "" '~' ""+ ""+ str(current_year)[2:4] +"" ""+ month_name +"" + "" '/' ""
        print '** Prefix **',prefix
        
        start_no = self.env['unique.outer.packing.number'].search([], limit=1, order='create_date desc')
        ...
        total_barcode = math.ceil(((1.0 / (self.product_id.inner_carton * self.product_id.inner_carton_qty)) * self.product_qty))
        ...
        for a in range(start,end):
            ...
            name = ""+ prefix +"" +"00"+ ""+ str(a) +"" + ""+ "" '~' ""+str(int(self.product_id.inner_carton * self.product_id.inner_carton_qty)) +""
            print 'NAME VALUE+++++',name
            curnt_datetime = datetime.datetime.now()
            user = self.env.user
            qty = 1.0
            self.env.cr.execute("INSERT INTO unique_outer_packing_number(name,create_date,responible_user,product_qty) VALUES (%s,%s,%s,%s)",(name,curnt_datetime,user.id,qty,))
            self.message = "" + str(int(total_barcode)) +""+ ' OUTER CARTON BARCODE GENERATED SUCCESSFULLY.'
