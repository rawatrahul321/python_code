...
class PosReturn(models.Model):
    ...
    def _prepare_invoice(self):
        invoice = super(PosReturn, self)._prepare_invoice()
        if self.return_ref:
    ...
    def action_pos_order_invoice(self):
        invoice = super(PosReturn, self).action_pos_order_invoice()
        for order in self:
            if order.return_ref:
                invoice['context'] = {'type':'out_refund'}
        return invoice

    @api.multi
    def action_view_invoice(self):
        invoice = super(PosReturn, self).action_view_invoice()
        for order in self:
            if order.return_ref:
                invoice['context'] = {'type':'out_refund'}
        return invoice
