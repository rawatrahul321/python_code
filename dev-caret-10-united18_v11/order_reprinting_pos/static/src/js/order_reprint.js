...
models.load_models({
            model: 'pos.order',
            fields: ['id', 'name', 'state', 'company_id', 'session_id', 'pos_reference', 'partner_id', 'amount_total', 'amount_tax', 'return_ref'],
            domain: function(self){ return [['date_order', '>=', priorDate], ['date_order', '<=', todayDate]]; },
            loaded: function (self, pos_orders) {
                var new_order_list = [];
                for (var i in pos_orders){
                    if (self.company.id === pos_orders[i].company_id[0] && pos_orders[i].state === "paid"){
...
var OldOrdersWidget = pos_screens.ScreenWidget.extend({
    ...
    renderElement: function () {
        ...
        this.$('.button.print').click(function(){
            rpc.query({model: 'pos.order', method: 'get_details', args:[self.pos_reference]}).then(function(id){
    ...
    render_list: function(orders){
        ...
        this.$('.order-list-contents').delegate('.print-button','click',function(event){
            ...
            var discount = 0;
            var image = [];
            rpc.query({model: 'pos.order', method: 'get_orderlines', args:[order_new.pos_reference]})
