...
class ftpConnection(models.Model):
        ...
        for order in orders:
            for line in order.order_line:
                if (line.product_id.is_fba and
                    ...
                    order.is_fbm_order = True
                elif line.product_id.is_kentucky_active_ants:
                    order.is_active_kants_order = True
            ...
            if order.is_active_kants_order:
                self.processKentuckyActiveAntsOrder(order)
                kants_orders += 1
            if order.is_active_ants_order:
                self.processActiveAntsOrder(order)
                ants_orders += 1
        self.exportOrders(export_products)
        message = ['%s Orders are FBM Orders and %s Orders are Ants Orders %s Orders are Kants Orders' % (
            fbm_orders, ants_orders, kants_orders)]
    ...
    def processKentuckyActiveAntsOrder(self, order):
            ...
            if api_token:
                ...
                if setting_data.get('result'):
                    ...
                    if setting_res.get('orderTypes'):
                        if conn.kants_order_type_id:
                            order_type_id = int(conn.kants_order_type_id)
                        else:
                            order_type_id = setting_res.get('orderTypes')[0]['id']
                    shipping_street = order.partner_shipping_id.street if order.partner_shipping_id else ''
                    billing_street = order.partner_invoice_id.street if order.partner_invoice_id else ''
                    if order.partner_shipping_id and order.partner_shipping_id.house_number and order.partner_shipping_id.street:
                        if order.partner_shipping_id.house_number in order.partner_shipping_id.street:
                            shipping_street = order.partner_shipping_id.street.replace(order.partner_shipping_id.house_number, '')
                    if order.partner_invoice_id and order.partner_invoice_id.house_number and order.partner_invoice_id.street:
                        if order.partner_invoice_id.house_number in order.partner_invoice_id.street:
                            billing_street = order.partner_invoice_id.street.replace(order.partner_invoice_id.house_number, '')
                    data = {
                    ...
                    kants_order = AuthorizeActiveAntsApi(conn.url, conn.user_name, conn.password).postOrders(api_token, data)
                    self.env['audit.log'].put_audit_log('Post Order to Kants',
                        'Success' if kants_order.get('messageCode') == 'OK' else 'Failed', kants_order, '')
                    if kants_order.get('messageCode') == 'OK' or '(%s) already exists'%order.name in kants_order.get('message'):
                        order.write({'state': 'kants_order'})
                    logger.info(_("Kentucky Active Ants Orders..., %s" %(kants_order)))
